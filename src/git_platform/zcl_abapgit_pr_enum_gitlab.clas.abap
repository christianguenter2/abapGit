...
CLASS zcl_abapgit_pr_enum_gitlab IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    " https://gitlab.com/api/v4/users/christianguenter/projects/

    mv_user = iv_user.
    mv_repo = iv_repo.

    mv_repo_url   = |https://gitlab.com/api/v4/users/{ iv_user }/projects|.
    mi_http_agent = ii_http_agent.
*    mi_http_agent->global_headers( )->set(
*      iv_key = 'Accept'
*      iv_val = 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8' ).
  ...
  METHOD fetch_repo_by_url.
    ...
    TRY.
        ...
        lt_items = rs_info-repo_json->members( '/' ).

        LOOP AT lt_items ASSIGNING FIELD-SYMBOL(<item>).
          DATA(name) = rs_info-repo_json->get( |/{ <item> }/path| ).

          IF name = mv_repo.
            DATA(id) = rs_info-repo_json->get( |/{ <item> }/id| ).
...
*        ENDIF.
        ...
        " https://gitlab.com/api/v4/projects/15549286

        li_response = mi_http_agent->request( |https://gitlab.com/api/v4/projects/{ id }| ).

        rs_info-repo_json = li_response->json( ).
        DATA(headers) = li_response->headers( ). " for debug

        DATA(forked_url) = rs_info-repo_json->get( '/forked_from_project/http_url_to_repo' ).

*        lv_pull_url = clean_url( rs_infk-repo_json->get( '/pulls_url' ) ).
